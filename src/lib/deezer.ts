...
export async function searchSpotifyPlaylists(query: string, limit: number = 10): Promise<SpotifyPlaylist[]> {
  try {
    ...
    console.error('Spotify playlist search failed:', e);
    return [];
  }
}

// Get Spotify playlists for an artist
export async function getSpotifyArtistPlaylists(artistName: string): Promise<SpotifyPlaylist[]> {
  try {
    const { data, error } = await supabase.functions.invoke('spotify-search', {
      body: { action: 'artistPlaylists', artistName },
    });

    if (error) {
      console.error('Spotify artist playlists error:', error);
      return [];
    }
    
    return data?.playlists || [];
  } catch (e) {
    console.error('Spotify artist playlists failed:', e);
    return [];
  }
}
